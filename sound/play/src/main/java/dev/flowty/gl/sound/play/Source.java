...
public class Source {
  ...
  /**
   * The priority of this {@link Source}. Lower priority {@link Source}s are recycled
   * preferentially
   */
  ...
  Source() {
    ...
    if (e != AL10.AL_NO_ERROR) {
      throw new IllegalStateException("Problem creating a source " + e);
    }
  }
  ...
  public Source lock(Object owner) {
    if (lock != null) {
      throw new IllegalStateException(
          "Attempt to lock source already locked by " + lock);
    }
  ...
  public Source release(Object owner) {
    if (lock != owner) {
      throw new IllegalStateException(
          "Attempt to release with wrong lock: " + lock + " != " + owner);
    }
  ...
  /**
   * @return <code>true</code> if locked, <code>false</code> if free for use
   */
   ...
   *
   * @param s the new sound, or null for no sound
   * @return This {@link Source}
   */
  public Source bindSound(Sound s) {
    ...
    } else if (boundBuffer != s.getBufferName()) {
   ...
   *
   * @param loop <code>true</code> to loop playback
   * @return This {@link Source}
   */
  public Source setLoop(boolean loop) {
    AL10.alSourcei(id, AL10.AL_LOOPING, loop
        ? AL10.AL_TRUE
        : AL10.AL_FALSE);
    return this;
  }
   ...
   * Sets the gain
   *
   * @param f will be clamped to >= 0
   * @return this {@link Source}
   */
  public Source setGain(float f) {
    AL10.alSourcef(id, AL10.AL_GAIN, Math.max(0, f));
    return this;
  }
   ...
   *
   * @param pitch will be clamped to >= 0
   * @return this {@link Source}
   */
  public Source setPitch(float pitch) {
    AL10.alSourcef(id, AL10.AL_PITCH, Math.max(0, pitch));
    return this;
  }
