package dev.flowty.gl.sound.generation.post;

import dev.flowty.gl.sound.generation.PostProcess;
...
public class CombFilter implements PostProcess {
  ...
  public void process(ShortBuffer data, int sampleRate) {
    ShortBuffer copy = ByteBuffer.allocateDirect(data.capacity() * 2).order(data.order())
        .asShortBuffer();
    copy.put(data);
    data.rewind();
    copy.rewind();
