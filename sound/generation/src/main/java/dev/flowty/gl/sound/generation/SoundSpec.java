...
public class SoundSpec {
  ...
  /**
   * The number of supersamples to calculate for every sample. More is slower, but less and some
   * waveforms sound like ass
   */
  ...
  /**
   * Generates the sound from this {@link SoundSpec}'s parameters, in 16-bit little-endian format
   *
   * @param sampleRate The number of samples to take per second
  ...
  public ByteBuffer generate(int sampleRate) {
    ...
    ByteBuffer buffer = ByteBuffer.allocateDirect(samples * 2).order(ByteOrder.LITTLE_ENDIAN);
