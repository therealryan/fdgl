...
/**
 * Call it an unordered list or a multiset, this collection is defined by oxymorons. Find the
 * original <a href=http://riven8192.blogspot.com/ 2009/08/bag-unordered-list-fast-remove.html>over
 * here</a>
 *
...
public class Bag<T> {
  ...
  /**
   * @param space initial size of bag
   */
  ...
  public void putAll(Bag<T> bag) {
    ...
    data = ensure(data, size + bag.size, 1.75f);
    System.arraycopy(bag.data, 0, data, size, bag.size);
    size += bag.size;
  }
  ...
  /**
   * Tries to shrink the internal array. If your {@link Bag} holds a lot of elements temporarily and
   * you're concerned with memory usage, it might be worthwhile to call this after a sequence of
   * {@link #take(int)} calls
   */
  @SuppressWarnings("unchecked")
  public void shrink() {
  ...
  /**
   * The maximum number of elements allowed before the internal array will be grown
   *
